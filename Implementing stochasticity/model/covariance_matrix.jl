...
function correlation_matrix(AS::AddStochasticity
    ...
    elseif size(corrmat, 1) < length(AS.stochspecies)
        ls = length(AS.stochspecies)
        throw(ArgumentError("The provided matrix is not big enough (needs to be $ls x $ls)"))
    end

    for i in corrmat
        if i > 1.0 || i < -1.0
            throw(ArgumentError("All elements in the correlation matrix must be between -1 and 1"))
        end
    end
    ...
    @assert isposdef(corrmat)

    esc = hcat(I(richness(FW)), zeros(richness(FW),length(AS.stochspecies)))
    dsc = hcat(zeros(length(AS.stochspecies),richness(FW)), corrmat)
    rmat = vcat(esc,dsc)
...
function sd_matrix(FW::FoodWeb, AS::AddStochasticity)
    sdmat = zeros(richness(FW) + length(AS.stochspecies), richness(FW) + length(AS.stochspecies))
...
function cov_matrix(FW::FoodWeb, AS::AddStochasticity
    , corrmat::Union{Matrix{Float64}, Nothing} = nothing)
    sdmat = sd_matrix(FW, AS)
    rmat = correlation_matrix(AS, corrmat)
